...
class CssModule extends _webpack.default.Module {
  constructor(dependency) {
    ...
    let home =  path.resolve('.');
    let split = dependency.identifier.split(home);
    // because this plug in depends on two other loaders, it will be the last entry in this 
    // that contains the original file name
    this.originalFile = split[split.length-1]; 
...
class MiniCssExtractPlugin {
  constructor(options = {}) {
    this.options = Object.assign({
      ...
      moduleFilename: (chunkData, filename) =>  {
        if(filename)
          return filename
    ...
    compiler.hooks.thisCompilation.tap(pluginName, compilation => {
      ...
      compilation.hooks.renderManifest.tap(pluginName, (result, {
        ...
        if (renderedModules.length > 0) {
          if(this.options.splitChunks){
            if(!modules){
              modules = this.generateModuleList(compilation, chunk, renderedModules);
              sources = this.renderAsMultiSource(modules, compilation.runtimeTemplate.requestShortener)
            }
            ...
            for (const key in sources) {
              ...
              const temp = {
                ...
                filenameTemplate: (chunkData ) => this.options.moduleFilename(chunkData, key),
                pathOptions: {
                  chunk,
            ...
            const temp = {
              render: () => {
                if(!modules){
                  modules = this.generateModuleList(compilation, chunk, renderedModules);
                  return this.renderAsSingleSource(modules, compilation.runtimeTemplate.requestShortener)
                }
              },
  ...
  generateModuleList(compilation, chunk, modules) {
    let usedModules;
  ...
  renderAsSingleSource(usedModules, requestShortener){
  ...
  renderAsMultiSource(usedModules, requestShortener){
    ...
    for (const m of usedModules) {
      ...
      let externalsSource = new ConcatSource();
      let previous = referenceMap[m.originalFile]
      if(previous){
        source = previous.source;
        externalsSource = previous.externalsSource;
      }
      ...
      referenceMap[m.originalFile] = {
        source: source,
        externalsSource: externalsSource
      }
    }

    let concnatSources = {}
    for (const key in referenceMap) {
      concatSources[key] = new ConcatSource(referenceMap[key].externalsSource, referenceMap[key].source)
    }
